...
build:
    ...
    # 2. UniFFI bindings (from debug dylib)
    ...
    if [ "{{codesign_identity}}" = "-" ]; then
        ...
        codesign --force --sign "{{codesign_identity}}" --options runtime --entitlements Wrenflow.entitlements --timestamp "$BUNDLE"
    fi
    echo "Built $BUNDLE"
...
# Build release .app bundle (arm64, signed)
release:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    # 2. UniFFI bindings (need debug dylib for bindgen, but link against release .a)
    mkdir -p Sources/Generated FFIModule
    cd {{rust_dir}}
    cargo build -p wrenflow-ffi  # debug build for uniffi-bindgen
    cargo run -p uniffi-bindgen generate \
        --library target/debug/libwrenflow_ffi.dylib \
        --language swift \
        --out-dir ../Sources/Generated
    ...
    # 3. Swift release (links against core/target/release/libwrenflow_ffi.a)
